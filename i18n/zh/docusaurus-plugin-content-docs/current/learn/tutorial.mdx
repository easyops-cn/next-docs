...
<img
  src="/img/docs/my-todos-light.png"
  width={309}
...
Here you're using a built-in [provider brick](../concepts/provider-bricks.md) `basic.http-request` which can fetch arbitrary HTTP API. It accepts a URL as its first argument. For simplicity, a static JSON file is used in this tutorial.
...
But the clear button is not working as you expected, it will only clear `Hard work` which is initialized with `done: true`. If the user clicked other todos as done, they will not get cleared though.
